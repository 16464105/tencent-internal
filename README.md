...
Store the CodeBuddy key and edit the model catalog on **Settings → CodeBuddy**, or store the key through the credentials service (`TENCENT_CODEBUDDY_API_KEY`). First-run does not prompt for the key. The package ships a `dsh.client` settings page, so a git install does not need a Models-page layout.
